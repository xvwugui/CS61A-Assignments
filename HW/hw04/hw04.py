...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    return True if is_planet(m) else length(left(m)) * total_mass(end(left(m))) == length(right(m)) * total_mass(end(right(m))) and balanced(end(left(m))) and balanced(end(right(m)))
